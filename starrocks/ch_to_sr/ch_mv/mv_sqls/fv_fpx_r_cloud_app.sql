/*
{
    "type": "sp_mv",
    "version": "070600.3329",
    "name": "fv_fpx_r_cloud_app",
    "datasource_mv": "fv_fpx_r_cloud_app_5min_mv_sp$SPID"
}
*/
DROP TABLE IF EXISTS siem.fv_fpx_r_cloud_app_5min_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fpx_r_cloud_app_hour_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fpx_r_cloud_app_day_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fpx_r_cloud_app_5min_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fpx_r_cloud_app_hour_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fpx_r_cloud_app_day_sp$SPID;

CREATE TABLE IF NOT EXISTS siem.fv_fpx_r_cloud_app_5min_sp$SPID (
       timescale DateTime,
       dvid Int32,
       _adomoid UInt64,
...
ORDER BY (_adomoid, dvid, timescale, srcintfrole, dstintfrole,
          app_group, clouduser, srcip, dstip)
PARTITION BY toYYYYMMDD(timescale)
SETTINGS index_granularity = 8192, allow_nullable_key = 1 $STORAGE;


CREATE TABLE IF NOT EXISTS siem.fv_fpx_r_cloud_app_hour_sp$SPID AS siem.fv_fpx_r_cloud_app_5min_sp$SPID;

CREATE TABLE IF NOT EXISTS siem.fv_fpx_r_cloud_app_day_sp$SPID AS siem.fv_fpx_r_cloud_app_5min_sp$SPID;

CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_fpx_r_cloud_app_5min_mv_sp$SPID
TO siem.fv_fpx_r_cloud_app_5min_sp$SPID
AS SELECT 
    ...
    FROM (
      ...
      WHERE bitAnd(logflag, 4) >0 AND _devlogtype = 15000
)
GROUP BY _adomoid, dvid, timescale, srcintfrole, dstintfrole,
          app_group, clouduser, srcip, dstip;

CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_fpx_r_cloud_app_hour_mv_sp$SPID
TO siem.fv_fpx_r_cloud_app_hour_sp$SPID
AS SELECT
...
FROM (
    ...
    FROM siem.fv_fpx_r_cloud_app_5min_sp$SPID
    GROUP BY _adomoid, dvid, timescale, srcintfrole, dstintfrole,
             app_group, clouduser, srcip, dstip
)
GROUP BY _adomoid, dvid, timescale, srcintfrole, dstintfrole,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_fpx_r_cloud_app_day_mv_sp$SPID
TO siem.fv_fpx_r_cloud_app_day_sp$SPID
AS SELECT
...
FROM (
    SELECT
       ...
       sumMerge(session_block_state) AS session_block,
       sumMerge(sessions_state) AS sessions
    FROM siem.fv_fpx_r_cloud_app_hour_sp$SPID
    GROUP BY _adomoid, dvid, timescale, srcintfrole, dstintfrole,
             app_group, clouduser, srcip, dstip
)
GROUP BY _adomoid, dvid, timescale, srcintfrole, dstintfrole,
...
ALTER TABLE siem.fv_fpx_r_cloud_app_day_mv_sp$SPID MODIFY COMMENT '$VERSION';
