/*
{
    "type": "sp_mv",
    "version": "070600.3329",
    "name": "fv_ffw_e_unauthorized",
    "datasource_mv": "fv_ffw_e_unauthorized_5min_mv_sp$SPID"
}
*/
DROP TABLE IF EXISTS siem.fv_ffw_e_system_event_5min_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_ffw_e_unauthorized_hour_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_ffw_e_unauthorized_day_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_ffw_e_unauthorized_5min_sp$SPID;
DROP TABLE IF EXISTS siem.fv_ffw_e_unauthorized_hour_sp$SPID;
DROP TABLE IF EXISTS siem.fv_ffw_e_unauthorized_day_sp$SPID;

CREATE TABLE IF NOT EXISTS siem.fv_ffw_e_unauthorized_5min_sp$SPID (
    _adomoid UInt64,
    dvid Int32,
    timescale DateTime,
...
CREATE TABLE IF NOT EXISTS siem.fv_ffw_e_unauthorized_hour_sp$SPID AS siem.fv_ffw_e_unauthorized_5min_sp$SPID;


CREATE TABLE IF NOT EXISTS siem.fv_ffw_e_unauthorized_day_sp$SPID AS siem.fv_ffw_e_unauthorized_5min_sp$SPID;


CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_ffw_e_unauthorized_5min_mv_sp$SPID
TO siem.fv_ffw_e_unauthorized_5min_sp$SPID
AS SELECT 
    _adomoid,
    dvid,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_ffw_e_unauthorized_hour_mv_sp$SPID
TO siem.fv_ffw_e_unauthorized_hour_sp$SPID
AS SELECT 
    _adomoid,
    dvid,
...
FROM (
      ...
      FROM siem.fv_ffw_e_unauthorized_5min_sp$SPID
      GROUP BY _adomoid, dvid, timescale,
         `user`, subtype, logintype, ui, vpntunnel,
         policyid, ssid, srcip, method, dstip, remip,
         xauthuser, initiator, stamac, interface, community, logid
...
CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_ffw_e_unauthorized_day_mv_sp$SPID
TO siem.fv_ffw_e_unauthorized_day_sp$SPID
AS SELECT 
    _adomoid,
    dvid,
...
FROM (
      SELECT
        ...
        logid,
        sumMerge(total_num_state) AS total_num
      FROM siem.fv_ffw_e_unauthorized_hour_sp$SPID
      GROUP BY _adomoid, dvid, timescale,
         `user`, subtype, logintype, ui, vpntunnel,
         policyid, ssid, srcip, method, dstip, remip,
         xauthuser, initiator, stamac, interface, community, logid
...
ALTER TABLE siem.fv_ffw_e_unauthorized_day_mv_sp$SPID MODIFY COMMENT '$VERSION';
