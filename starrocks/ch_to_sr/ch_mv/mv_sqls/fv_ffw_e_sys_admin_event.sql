/*
{
    "type": "sp_mv",
    "version": "070600.3329",
    "name": "fv_ffw_e_sys_admin_event",
    "datasource_mv": "fv_ffw_e_sys_admin_event_5min_mv_sp$SPID"
}
*/
DROP TABLE IF EXISTS siem.fv_ffw_e_sys_admin_event_5min_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_ffw_e_sys_admin_event_hour_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_ffw_e_sys_admin_event_day_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_ffw_e_sys_admin_event_5min_sp$SPID;
DROP TABLE IF EXISTS siem.fv_ffw_e_sys_admin_event_hour_sp$SPID;
DROP TABLE IF EXISTS siem.fv_ffw_e_sys_admin_event_day_sp$SPID;

CREATE TABLE IF NOT EXISTS siem.fv_ffw_e_sys_admin_event_5min_sp$SPID (
    _adomoid UInt64,
    dvid Int32,
    timescale DateTime,
...
CREATE TABLE IF NOT EXISTS siem.fv_ffw_e_sys_admin_event_hour_sp$SPID AS siem.fv_ffw_e_sys_admin_event_5min_sp$SPID;


CREATE TABLE IF NOT EXISTS siem.fv_ffw_e_sys_admin_event_day_sp$SPID AS siem.fv_ffw_e_sys_admin_event_5min_sp$SPID;

CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_ffw_e_sys_admin_event_5min_mv_sp$SPID
TO siem.fv_ffw_e_sys_admin_event_5min_sp$SPID
AS SELECT
    _adomoid,
    dvid,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_ffw_e_sys_admin_event_hour_mv_sp$SPID
TO siem.fv_ffw_e_sys_admin_event_hour_sp$SPID
AS SELECT
    _adomoid,
    dvid,
...
FROM (
      ...
      FROM siem.fv_ffw_e_sys_admin_event_5min_sp$SPID
      GROUP BY _adomoid, dvid, timescale, f_user, event_name, severity_fgt, severity, level, subtype
)
GROUP BY _adomoid, dvid, timescale, f_user, event_name, severity_fgt, severity, level, subtype;

CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_ffw_e_sys_admin_event_day_mv_sp$SPID
TO siem.fv_ffw_e_sys_admin_event_day_sp$SPID
AS SELECT
    _adomoid,
    dvid,
...
FROM (
      SELECT
          ...
          sumMerge(total_duration_state) AS total_duration,
          sumMerge(total_change_state) AS total_change
      FROM siem.fv_ffw_e_sys_admin_event_hour_sp$SPID
      GROUP BY _adomoid, dvid, timescale, f_user, event_name, severity_fgt, severity, level, subtype
)
GROUP BY _adomoid, dvid, timescale, f_user, event_name, severity_fgt, severity, level, subtype;

ALTER TABLE siem.fv_ffw_e_sys_admin_event_day_mv_sp$SPID MODIFY COMMENT '$VERSION';
