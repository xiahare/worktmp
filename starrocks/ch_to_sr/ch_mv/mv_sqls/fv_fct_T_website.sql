/*
{
    "type": "sp_mv",
    "version": "070600.3329",
    "name": "fv_fct_T_website",
    "datasource_mv": "fv_fct_T_website_5min_mv_sp$SPID"
}
*/
DROP TABLE IF EXISTS siem.fv_fct_T_website_5min_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fct_T_website_hour_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fct_T_website_day_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fct_T_website_5min_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fct_T_website_hour_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fct_T_website_day_sp$SPID;

CREATE TABLE IF NOT EXISTS siem.fv_fct_T_website_5min_sp$SPID (
...
ORDER BY (_adomoid, dvid, timescale,
         fgtserial, emsserial, domain, f_user)
PARTITION BY toYYYYMMDD(timescale)
SETTINGS index_granularity = 8192, allow_nullable_key = 1 $STORAGE;

CREATE TABLE IF NOT EXISTS siem.fv_fct_T_website_hour_sp$SPID AS siem.fv_fct_T_website_5min_sp$SPID;

CREATE TABLE IF NOT EXISTS siem.fv_fct_T_website_day_sp$SPID AS siem.fv_fct_T_website_5min_sp$SPID;

CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_fct_T_website_5min_mv_sp$SPID
TO siem.fv_fct_T_website_5min_sp$SPID
AS SELECT _adomoid, dvid, fv_timescale_func(itime, 300, 0) AS timescale,
       ...
       FROM (
           SELECT
               ...
               $LOGFIELD-remotename,
               $LOGFIELD-utmaction,
               root_domain(remotename) AS domain,
               $LOGFIELD-sentbyte,
               $LOGFIELD-rcvdbyte,
               sentbyte + rcvdbyte AS bandwidth,
               rcvdbyte AS traffic_in,
               sentbyte AS traffic_out,
               toInt64(1) AS sessions,
               toInt64(0) AS session_block
          FROM siem.ulog_sp$SPID
          WHERE _devlogtype = 3016 AND utmevent = 'webfilter' AND nullif(remotename,'') IS NOT NULL
)
GROUP BY _adomoid, timescale, dvid, fgtserial, emsserial,
         domain, f_user;

CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_fct_T_website_hour_mv_sp$SPID
TO siem.fv_fct_T_website_hour_sp$SPID
...
FROM (
   SELECT _adomoid, dvid, timescale,
    ...
    FROM siem.fv_fct_T_website_5min_sp$SPID
    GROUP BY _adomoid, timescale, dvid, fgtserial, emsserial,
             domain, f_user
)
GROUP BY _adomoid, timescale, dvid, fgtserial, emsserial,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_fct_T_website_day_mv_sp$SPID
TO siem.fv_fct_T_website_day_sp$SPID
AS SELECT _adomoid, dvid,
...
FROM (
   SELECT _adomoid, dvid, timescale,
       ...
       sumMerge(session_block_state) AS session_block,
       sumMerge(sessions_state) AS sessions
    FROM siem.fv_fct_T_website_hour_sp$SPID
    GROUP BY _adomoid, timescale, dvid, fgtserial, emsserial,
             domain, f_user
)
GROUP BY _adomoid, timescale, dvid, fgtserial, emsserial,
...
ALTER TABLE siem.fv_fct_T_website_day_mv_sp$SPID MODIFY COMMENT '$VERSION';
