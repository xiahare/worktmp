/*
{
    "type": "sp_mv",
    "version": "070600.3329",
    "name": "fv_fpx_t_threat",
    "datasource_mv": "fv_fpx_t_threat_5min_mv_sp$SPID"
}
*/
DROP TABLE IF EXISTS siem.fv_fpx_t_threat_5min_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fpx_t_threat_hour_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fpx_t_threat_day_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fpx_t_threat_5min_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fpx_t_threat_hour_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fpx_t_threat_day_sp$SPID;

CREATE TABLE IF NOT EXISTS siem.fv_fpx_t_threat_5min_sp$SPID (
...
ORDER BY (_adomoid, dvid, timescale, srcintfrole, dstintfrole,
         ...
         dstcountry, dstip, dstuuid, dstintf, dstowner, policyid, policytype)
PARTITION BY toYYYYMMDD(timescale)
SETTINGS index_granularity = 8192, allow_nullable_key = 1 $STORAGE;

CREATE TABLE IF NOT EXISTS siem.fv_fpx_t_threat_hour_sp$SPID AS siem.fv_fpx_t_threat_5min_sp$SPID;

CREATE TABLE IF NOT EXISTS siem.fv_fpx_t_threat_day_sp$SPID AS siem.fv_fpx_t_threat_5min_sp$SPID;

CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_fpx_t_threat_5min_mv_sp$SPID
TO siem.fv_fpx_t_threat_5min_sp$SPID
    ...
    FROM ( 
      SELECT *,
       ...
       FROM (
          ...
          WHERE _devlogtype = 15010 AND threats IS NOT NULL)
)
GROUP BY _adomoid, dvid, srcintfrole, dstintfrole, timescale,
     threat_s, threattype_s, threatlevel_s,
     app_group, f_user, srcip, srcuuid, srcintf, srcmac,
     dstcountry, dstip, dstuuid, dstintf, dstowner,
     policymode, policyid, policytype, poluuid;


CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_fpx_t_threat_hour_mv_sp$SPID
TO siem.fv_fpx_t_threat_hour_sp$SPID
AS SELECT _adomoid, dvid, srcintfrole, dstintfrole,
...
FROM (
   SELECT _adomoid, dvid, srcintfrole, dstintfrole,
    ...
    FROM siem.fv_fpx_t_threat_5min_sp$SPID
    GROUP BY _adomoid, dvid, srcintfrole, dstintfrole, timescale,
         threat_s, threattype_s, threatlevel_s,
         app_group, f_user, srcip, srcuuid, srcintf, srcmac,
         dstcountry, dstip, dstuuid, dstintf, dstowner,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_fpx_t_threat_day_mv_sp$SPID
TO siem.fv_fpx_t_threat_day_sp$SPID
AS SELECT _adomoid, dvid, srcintfrole, dstintfrole,
...
FROM (
   SELECT _adomoid, dvid, srcintfrole, dstintfrole,
       ...
       sumMerge(incident_block_state) AS incident_block,
       sumMerge(incident_state) AS incident
    FROM siem.fv_fpx_t_threat_hour_sp$SPID
    GROUP BY _adomoid, dvid, srcintfrole, dstintfrole, timescale,
         threat_s, threattype_s, threatlevel_s,
         app_group, f_user, srcip, srcuuid, srcintf, srcmac,
         dstcountry, dstip, dstuuid, dstintf, dstowner,
...
ALTER TABLE siem.fv_fpx_t_threat_day_mv_sp$SPID MODIFY COMMENT '$VERSION';
