/*
{
    "type": "sp_mv",
    "version": "070600.3348",
    "name": "fv_fpx_t_src_dst",
    "datasource_mv": "fv_fpx_t_src_dst_5min_mv_sp$SPID"
}
*/
DROP TABLE IF EXISTS siem.fv_fpx_t_src_dst_5min_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fpx_t_src_dst_hour_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fpx_t_src_dst_day_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fpx_t_src_dst_5min_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fpx_t_src_dst_hour_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fpx_t_src_dst_day_sp$SPID;

CREATE TABLE IF NOT EXISTS siem.fv_fpx_t_src_dst_5min_sp$SPID (
    ...
    srcuuid Nullable(UUID) CODEC(LZ4), srcinetsvc Nullable(String) CODEC(LZ4),
    clientdevicetags Nullable(String),
    srcmac_state AggregateFunction(max, Nullable(String)), dev_src_state AggregateFunction(max, Nullable(String)),
    dstmac_state AggregateFunction(max, Nullable(String)), devtype_state AggregateFunction(max, Nullable(String)),
    app_group Nullable(String), rulename Nullable(String), vwlid Nullable(UInt32), domain Nullable(String), hostname Nullable(String),
    catdesc LowCardinality(Nullable(String)), policymode LowCardinality(Nullable(String)), policyid Nullable(UInt32),
	policytype LowCardinality(Nullable(String)), poluuid Nullable(UUID), policyname Nullable(String), service LowCardinality(Nullable(String)),
    accessproxy Nullable(String), fctuid Nullable(UUID), avatar_state  AggregateFunction(max, Nullable(String)),
...
ORDER BY (_adomoid, dvid, timescale, srcintfrole, dstintfrole, srcip, dstip,
         epid, f_user, f_group, srcintf, dstintf, dstowner, dstcountry,
         srcuuid, srcinetsvc, dstuuid, app_group, rulename, vwlid, flags, d_flags,
         catdesc, policymode, policyid, policytype, poluuid,
         policyname, service, domain, hostname, fctuid,
         accessproxy, clientdevicetags)
PARTITION BY toYYYYMMDD(timescale)
SETTINGS index_granularity = 8192, allow_nullable_key = 1 $STORAGE;


CREATE TABLE IF NOT EXISTS siem.fv_fpx_t_src_dst_hour_sp$SPID AS siem.fv_fpx_t_src_dst_5min_sp$SPID;


CREATE TABLE IF NOT EXISTS siem.fv_fpx_t_src_dst_day_sp$SPID AS siem.fv_fpx_t_src_dst_5min_sp$SPID;


CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_fpx_t_src_dst_5min_mv_sp$SPID
TO siem.fv_fpx_t_src_dst_5min_sp$SPID
AS SELECT _adomoid, dvid, srcintfrole, dstintfrole, 
       ...
       `group` AS f_group, srcip, srcintf, srcuuid, srcinetsvc, maxState(srcmac) AS srcmac_state,
       clientdevicetags,
       maxState(dev_src) AS dev_src_state, dstip, dstintf, dstuuid, dstowner, dstcountry,
       maxState(dstmac) AS dstmac_state, maxState(devtype) AS devtype_state, flags, d_flags,
       app_group, rulename, vwlid, domain, hostname, catdesc, policymode, policyid, policytype, poluuid, policyname, service,
       accessproxy, fctuid, maxState(avatar) AS avatar_state,
    ...
    FROM (
       SELECT
           ...
           $LOGFIELD-srcinetsvc,
           $LOGFIELD-srcmac,
           $LOGFIELD-srcname, coalesce(srcname, srcmac) AS dev_src,
           $LOGFIELD-dstip,
           $LOGFIELD-dstintf, 
           $LOGFIELD-dstuuid, 
           $LOGFIELD-dstowner, 
           ...
           $LOGFIELD-policyname,
           $LOGFIELD-service,
            coalesce(vwlname,vwlservice) AS rulename,
           app_group_name(app) AS app_group,
           $LOGFIELD-hostname, logflag,
           $LOGFIELD-unauthuser,
      ...
      WHERE bitAnd(logflag, bitOr(1, 32)) > 0 AND _devlogtype = 15010
)
GROUP BY _adomoid, dvid, srcintfrole, dstintfrole, timescale,
     epid, f_user, f_group, srcip, srcintf, dstip, dstintf, dstowner, dstcountry,
     srcuuid, srcinetsvc, dstuuid, app_group, rulename, vwlid, flags, d_flags,
     catdesc, policymode, policyid, policytype, poluuid,
     policyname, service, domain, hostname, fctuid,
     accessproxy, clientdevicetags;
...
CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_fpx_t_src_dst_hour_mv_sp$SPID
TO siem.fv_fpx_t_src_dst_hour_sp$SPID
AS SELECT dvid, srcintfrole, dstintfrole,
       ...
       f_group, srcip, srcintf, srcuuid, srcinetsvc, maxState(srcmac) AS srcmac_state,
       maxState(dev_src) AS dev_src_state,
       maxState(dstmac) AS dstmac_state, maxState(devtype) AS devtype_state,
       app_group, rulename, vwlid, domain, hostname, catdesc, policymode, policyid, policytype, poluuid, policyname, service,
       clientdevicetags, accessproxy, fctuid, maxState(avatar) AS avatar_state,
...
FROM (
    SELECT
        timescale, epid, f_user, f_group, srcip, srcintf, srcuuid, srcinetsvc,
        _adomoid, flags, d_flags,
        dstip, dstintf, dstuuid, dstowner, dstcountry,
        app_group, rulename, vwlid, domain, hostname, catdesc, policymode, policyid, policytype, poluuid, policyname, service,
        clientdevicetags, accessproxy, dstintfrole, fctuid, dvid, srcintfrole,
        maxMerge(srcmac_state) AS srcmac,
        maxMerge(dev_src_state) AS dev_src,
    ...
    FROM siem.fv_fpx_t_src_dst_5min_sp$SPID
    GROUP BY _adomoid, dvid, srcintfrole, dstintfrole, timescale,
         epid, f_user, f_group, srcip, srcintf, dstip, dstintf, dstowner, dstcountry,
         srcuuid, srcinetsvc, dstuuid, app_group, rulename, vwlid, flags, d_flags,
         catdesc, policymode, policyid, policytype, poluuid,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_fpx_t_src_dst_day_mv_sp$SPID
TO siem.fv_fpx_t_src_dst_day_sp$SPID
AS SELECT _adomoid, dvid, srcintfrole, dstintfrole,
...
FROM (
    SELECT
        ...
        sumMerge(sessions_state) AS sessions
    FROM siem.fv_fpx_t_src_dst_hour_sp$SPID
    GROUP BY _adomoid, dvid, srcintfrole, dstintfrole, timescale,
         epid, f_user, f_group, srcip, srcintf, dstip, dstintf, dstowner, dstcountry,
         srcuuid, srcinetsvc, dstuuid, app_group, rulename, vwlid, flags, d_flags,
         catdesc, policymode, policyid, policytype, poluuid,
...
ALTER TABLE siem.fv_fpx_t_src_dst_day_mv_sp$SPID MODIFY COMMENT '$VERSION';
