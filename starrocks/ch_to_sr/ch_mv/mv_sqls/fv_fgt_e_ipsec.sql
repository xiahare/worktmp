/*
{
    ...
    "version": "070600.3397",
    "name": "fv_fgt_e_ipsec",
    "datasource_mv": "fv_fgt_e_ipsec_5min_mv_sp$SPID"
}
*/
DROP TABLE IF EXISTS siem.fv_fgt_e_ipsec_5min_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fgt_e_ipsec_hour_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fgt_e_ipsec_day_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fgt_e_ipsec_5min_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fgt_e_ipsec_hour_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fgt_e_ipsec_day_sp$SPID;

CREATE TABLE IF NOT EXISTS siem.fv_fgt_e_ipsec_5min_sp$SPID (
       timescale DateTime,
       dvid Int32,
       _adomoid UInt64,
       ...
       max_duration_state AggregateFunction(max, Int32),
       max_advpnsc_state AggregateFunction(max, Nullable(Int8)),
       min_duration_state AggregateFunction(min, Int32),
...
CREATE TABLE IF NOT EXISTS siem.fv_fgt_e_ipsec_hour_sp$SPID AS siem.fv_fgt_e_ipsec_5min_sp$SPID;

CREATE TABLE IF NOT EXISTS siem.fv_fgt_e_ipsec_day_sp$SPID AS siem.fv_fgt_e_ipsec_5min_sp$SPID;


CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_fgt_e_ipsec_5min_mv_sp$SPID
TO siem.fv_fgt_e_ipsec_5min_sp$SPID
AS SELECT 
       ...
       maxState(advpnsc) AS max_advpnsc_state,
    ...
    FROM (
       SELECT
           ...
           $LOGFIELD-vpntunnel,
           dtime,
           coalesce($LOGFIELD_NOALIAS-duration,0) AS duration,
           $LOGFIELD-sentbyte,
           $LOGFIELD-rcvdbyte,
           $LOGFIELD-tunnelip,
           $LOGFIELD-action,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_fgt_e_ipsec_hour_mv_sp$SPID
TO siem.fv_fgt_e_ipsec_hour_sp$SPID
AS SELECT
...
FROM (
    ...
    FROM siem.fv_fgt_e_ipsec_5min_sp$SPID
    GROUP BY _adomoid, dvid, timescale, vpnusergroup, xauthuser,
        user, vpnname,  remip, locip, tunneltype, tunnelid, vpntunnel, vpntype
)
GROUP BY _adomoid, dvid, timescale, vpnusergroup, xauthuser,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_fgt_e_ipsec_day_mv_sp$SPID
TO siem.fv_fgt_e_ipsec_day_sp$SPID
AS SELECT
...
FROM (
    SELECT
       ...
       sumMerge(traffic_out_state) AS traffic_out,
       sumMerge(traffic_in_state) AS traffic_in
    FROM siem.fv_fgt_e_ipsec_hour_sp$SPID
    GROUP BY _adomoid, dvid, timescale, vpnusergroup, xauthuser,
        user, vpnname,  remip, locip, tunneltype, tunnelid, vpntunnel, vpntype
)
GROUP BY _adomoid, dvid, timescale, vpnusergroup, xauthuser,
...
ALTER TABLE siem.fv_fgt_e_ipsec_day_mv_sp$SPID MODIFY COMMENT '$VERSION';
