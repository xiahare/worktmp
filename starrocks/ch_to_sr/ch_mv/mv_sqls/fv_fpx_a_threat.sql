/*
{
    "type": "sp_mv",
    "version": "070600.3329",
    "name": "fv_fpx_a_threat",
    "datasource_mv": "fv_fpx_a_threat_5min_mv_sp$SPID"
}
*/
DROP TABLE IF EXISTS siem.fv_fpx_a_threat_5min_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fpx_a_threat_hour_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fpx_a_threat_day_mv_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fpx_a_threat_5min_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fpx_a_threat_hour_sp$SPID;
DROP TABLE IF EXISTS siem.fv_fpx_a_threat_day_sp$SPID;

CREATE TABLE IF NOT EXISTS siem.fv_fpx_a_threat_5min_sp$SPID (
    dvid Int32,
    _adomoid UInt64,
    timescale DateTime,
    threat LowCardinality(Nullable(String)),
    threattype LowCardinality(Nullable(String)),
    threatlevel Nullable(Int32),
...
ORDER BY (_adomoid, dvid, timescale, srcintfrole, dstintfrole,
          threat, threattype, threatlevel,
          f_user, srcip, srcintf, dstip, dstintf)
PARTITION BY toYYYYMMDD(timescale)
SETTINGS index_granularity = 8192, allow_nullable_key = 1 $STORAGE;

CREATE TABLE IF NOT EXISTS siem.fv_fpx_a_threat_hour_sp$SPID AS siem.fv_fpx_a_threat_5min_sp$SPID;

CREATE TABLE IF NOT EXISTS siem.fv_fpx_a_threat_day_sp$SPID AS siem.fv_fpx_a_threat_5min_sp$SPID;

CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_fpx_a_threat_5min_mv_sp$SPID
TO siem.fv_fpx_a_threat_5min_sp$SPID
AS SELECT
    dvid,
    _adomoid,
    ...
    FROM (
       SELECT dvid, itime,
           ...
           (CASE WHEN threatlevel=4 THEN bitAnd(crscore, 65535) ELSE 0 END) AS thwgt_cri,
           (CASE WHEN threatlevel=3 THEN bitAnd(crscore, 65535) ELSE 0 END) AS thwgt_hig,
           (CASE WHEN threatlevel=2 THEN bitAnd(crscore, 65535) ELSE 0 END) AS thwgt_med,
           (CASE WHEN threatlevel=1 THEN bitAnd(crscore, 65535) ELSE 0 END) AS thwgt_low,
           $LOGFIELD-crscore,
      ...
      WHERE threat IS NOT NULL AND _devlogtype = 15001 AND msg LIKE 'anomaly%'
)
GROUP BY _adomoid, dvid, srcintfrole, dstintfrole, timescale,
         threat, threattype, threatlevel,
         f_user, srcip, srcintf, dstip, dstintf, dstcountry,
         policymode, policyid, policytype, poluuid;

CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_fpx_a_threat_hour_mv_sp$SPID
TO siem.fv_fpx_a_threat_hour_sp$SPID
AS SELECT
    dvid,
    _adomoid,
    ...
    FROM (
      ...
      FROM siem.fv_fpx_a_threat_5min_sp$SPID
      GROUP BY _adomoid, dvid, srcintfrole, dstintfrole, timescale,
               threat, threattype, threatlevel,
               f_user, srcip, srcintf, dstip, dstintf, dstcountry,
               policymode, policyid, policytype, poluuid
...
CREATE MATERIALIZED VIEW IF NOT EXISTS siem.fv_fpx_a_threat_day_mv_sp$SPID
TO siem.fv_fpx_a_threat_day_sp$SPID
AS SELECT
    dvid,
    _adomoid,
    ...
    FROM (
        ...
        sumMerge(incident_state) AS incidents,
        sumMerge(incident_block_state) AS incident_block
      FROM siem.fv_fpx_a_threat_hour_sp$SPID
      GROUP BY _adomoid, dvid, srcintfrole, dstintfrole, timescale,
               threat, threattype, threatlevel,
               f_user, srcip, srcintf, dstip, dstintf, dstcountry,
               policymode, policyid, policytype, poluuid
...
ALTER TABLE siem.fv_fpx_a_threat_day_mv_sp$SPID MODIFY COMMENT '$VERSION';
