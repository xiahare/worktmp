...
WITH crt_dev_lst AS (
    SELECT epid,
           ...
           SUM(CASE WHEN threat_severity = 'Critical'
                        OR threat_severity = 'High' THEN 1 ELSE 0 END) AS vuln_num
    FROM (
        SELECT user_src,
               max_snapshot,
               epid,
        ...
        FROM (
            ...
            FROM (
                ...
                WHERE adomid = 3
                  AND (__start_time >= '2025-07-21 07:00:00'
                       AND __start_time <= '2025-07-21 17:29:59')

                UNION ALL

                SELECT adomid                                                     AS `adomid`,
                       toInt32(toInt32(`itime`) / 1800) * 1800                  AS `timestamp`,
                       data_sourcename,
                       user_src,
                       epid,
                       host_name,
        ...
        WHERE tt.max_snapshot = tt.snapshot
    ) t
...
FROM (
    (SELECT COUNT(DISTINCT epid) AS added_cnt, 0 AS deleted_cnt
     FROM crt_dev_lst t1
     WHERE epid NOT IN (SELECT epid FROM pre_dev_lst t2))
    UNION ALL
    (SELECT 0 AS added_cnt, COUNT(DISTINCT epid) AS deleted_cnt
     FROM pre_dev_lst t1
